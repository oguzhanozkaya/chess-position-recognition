...
\begin{frame}
  \SlideTitle{Final Results}
  \begin{tabularx}{\textwidth}{@{}lXXXX@{}}
    \toprule
    \textbf{Split} & \textbf{Square} & \textbf{Occupied} & \textbf{Empty} & \textbf{Board} \\
    \midrule
  ...
  \end{tabularx}
  \vspace{1em}
  {\color{BodyGrey}\small Final CUDA run: 3 epochs, batch size 192, image size 256, mixed precision. Test split: 20,000 boards, 1,280,000 squares, 27 square errors.}
\end{frame}

\begin{frame}
  \SlideTitle{Result Interpretation}
  \begin{itemize}
    \item Empty-square accuracy reached 100\% on train, validation, and test.
    \item Occupied-square accuracy stayed above 99.97\%, so the model did not rely on empty-square dominance.
    \item Board accuracy is the strict metric: one wrong square fails the board; test board accuracy still reached 99.875\%.
    \item Remaining test errors were concentrated in rare non-empty classes, especially black queen confusions.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item The project implements chess board position recognition end to end.
    ...
    \item The final held-out test run reached 99.9979\% square accuracy and 99.8750\% board accuracy.
  \end{itemize}
\end{frame}
